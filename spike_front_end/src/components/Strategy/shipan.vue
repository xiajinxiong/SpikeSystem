...
<div style="position: relative;width: 100%;background: #344b58 !important" >
    ...
    <el-table-column prop="annual_return" label="年化收益率" min-width="100">
    </el-table-column>
    <el-table-column prop="MDD" label="最大回撤率" min-width="100">
    </el-table-column >
    <el-table-column prop="volatility" label="波动率" min-width="100">
    </el-table-column>
    <el-table-column prop="sharpe" label="夏普比率" min-width="100">
    </el-table-column>
    <el-table-column prop="sortino" label="索定能比率" min-width="100">
    </el-table-column>
    <el-table-column prop="calmar" label="Calmar比率" min-width="100">
    </el-table-column>    
    </el-table>
    </div>
    <div :zIndex="1" style="height: 30px;position: relative;z-index: 999999;left:5%;padding-top:1%;">
     <el-switch
      v-model="value1"
      inactive-text="">
    </el-switch>
    ...
    <shipan-bar-chart :cookie="this.$route.params.id"></shipan-bar-chart>
</div>
...
import shipanBarChart from '@/components/Strategy/shipanBarChart'
import axios from 'axios'

export default {
  components: {
    shipanBarChart: shipanBarChart
  },
  props: {
    className: {
      type: String,
  ...
  data() {
    return {
      ...
      map: new Map(),
      real_strategy_reference_value: 1.0,
  ...
  watch: {
    ...
    value1: function (val, oldVal) {
      if (val == true) {
        ...
        let data0 = [...this.log_strategy_data];
        for (let i = 0; i < data0.length; i++) {
          data0[i] -= this.log_strategy_data[this.startIndex];
        }
        let data1 = [...this.log_benchmark_assets];
        for (let i = 0; i < data1.length; i++) {
          data1[i] -= this.log_benchmark_assets[this.startIndex];
        }
        let data2 = [...this.log_alpha_assets];
        for (let i = 0; i < data2.length; i++) {
          data2[i] -= this.log_alpha_assets[this.startIndex];
        }
  ...
  mounted() {
    ...
    axios.get(config.url + 'assets_by_account/' + this.$route.params.id).then(response => {
    ...
    this.starttime = 0;
    this.endtime = this.real_strategy_data.length - 1;
    ...
    initChart() {
      ...
      let options = {
        ...
        series: [{
          ...
          // below added by xia
          //data: [1.0, 1.014921772285378, 1.0135929034820714, 1.0169418936478978, 1.0240150241114814, 1.014932476947561, 1.0200792275461252, 1.0175337698858848, 1.03527216801265, 1.026154830199575, 1.0166919677448671, 0.9986755434665109, 0.9990840281853316, 1.0280536844373875, 1.0200072725921154, 1.0169758476216457, 1.033948420525008, 1.0640056593841674, 1.061306852213823, 1.0675366674356257, 1.0752984643457528, 1.077014313552694, 1.0729923933508614, 1.0849119480502851, 1.0901944493243525, 1.0908160975071794, 1.0934441683555196, 1.0902171351603245, 1.1160154696666411, 1.1189512060800193, 1.1175606574545953, 1.1363413924300148, 1.1399356810053654, 1.1542781544769243, 1.1513742137353762, 1.1530165817563038, 1.1768757519053306, 1.1747200130045083, 1.179319476124852, 1.1939336064934323, 1.2192303235759299, 1.2481016643990046, 1.2707409657263589, 1.3110006815063662, 1.3271767820099023, 1.3722596688967414, 1.3647171567565288, 1.3390038832072213, 1.2610684520658455, 1.2728213592319346, 1.3099603333212804, 1.3277638637728342, 1.3420714130053137, 1.3413745245357636, 1.276210237085672, 1.2818992507087796, 1.2990524135469952, 1.3353733920011166, 1.3338241761860041, 1.3526936140174282, 1.3850133464748704, 1.382876822520146, 1.412232883474331, 1.409867845096792, 1.4096418301968248, 1.417433215521484, 1.3866723720782386, 1.3553017593122556, 1.3598181146948738, 1.3744542654871759, 1.4006279570954319, 1.4139714700028747, 1.3870140180815183, 1.3700312984966727, 1.3928256165499184, 1.41109645014756, 1.4124768956149254, 1.379976330021213, 1.4080624279361915, 1.427487460288597, 1.4322933627027532, 1.4467379572843844, 1.4491044520516305, 1.4387137772070795, 1.4378577690301362, 1.4045716392534588, 1.410477850976207, 1.353013548310812, 1.3046672355043434, 1.3412983823330502, 1.3932521825916706, 1.3976826162128453, 1.3805024995341477, 1.390066816746419, 1.4023691982463957, 1.3957293323934272, 1.372036007362, 1.3873464450392181, 1.3484513220577519, 1.3406958449253652, 1.3162258412161962, 1.334759462089572, 1.331912808918939, 1.3729815140249952, 1.4138568246329994, 1.424881533838964, 1.4181034663271674, 1.4159019376141293, 1.4189782261064257]
          data: vm.real_strategy_relative_data        
        },
  ...
  computed: {
    list: function() {
      ...
      let endIndex = this.map.get(this.endtime) == null ? this.real_strategy_data.length  - 1:this.map.get(this.endtime);
      ...
      let annual_return = this.annual_return[startIndex][endIndex];
      let alpha_annual_return = this.alpha_annual_return[startIndex][endIndex];
      let strategy_MDD = this.MDD[startIndex][endIndex];
      let alphaMDD = this.alphaMDD[startIndex][endIndex];
      let volatility = this.volatility[startIndex][endIndex];
      let alpha_volatility = this.alpha_volatility[startIndex][endIndex];
      let Sharpe = this.Sharpe[startIndex][endIndex];
      let alpha_Sharpe = this.alpha_Sharpe[startIndex][endIndex];
      let sortino = this.sortino[startIndex][endIndex];
      let alpha_sortino = this.alpha_sortino[startIndex][endIndex];
      let calmar = this.calmar[startIndex][endIndex];
      let alpha_calmar = this.alpha_calmar[startIndex][endIndex];
      return [{name: '策略', annual_return: annual_return, MDD: strategy_MDD, volatility: volatility,
      sharpe: Sharpe, sortino: sortino, calmar: calmar}, 
      {name: '超额', annual_return: alpha_annual_return, MDD: alphaMDD, volatility: alpha_volatility,
      sharpe: alpha_Sharpe, sortino: sortino, calmar: alpha_calmar}];
    }
  }
}
</script>
